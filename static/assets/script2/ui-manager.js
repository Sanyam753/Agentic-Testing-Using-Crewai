...
export class UIManager {
    ...
    updateSystemHealth(data) {
        const statusElements = {
            'lmStudioStatus': data.lm_studio || 'unknown',
            'modelsStatus': data.models || 'unknown',
            'dataAccessStatus': data.data_access || 'unknown'
        };
        ...
        Object.entries(statusElements).forEach(([elementId, status]) => {
            ...
            if (element) {
                ...
                if (status === 'error' || status === 'unknown') {
                    element.title = elementId === 'lmStudioStatus' 
                        ? 'LM Studio may not be running or model may have crashed' 
        ...
        if (systemHealth) {
            const allHealthy = Object.values(statusElements).every(status => status === 'healthy');
            const hasErrors = Object.values(statusElements).some(status => status === 'error' || status === 'unknown');
            
            systemHealth.className = `health-indicator ${allHealthy ? 'healthy' : hasErrors ? 'error' : 'warning'}`;
            
            // Add health summary text
            ...
            if (healthText) {
                if (allHealthy) {
                    ...
                    healthText.textContent = 'System partially operational';
                }
            }
        }
    }
